 ...
 */
mainApp.directive('chartHeat', function () {
    return {
        restrict: 'E',
        templateUrl: "directives/chart_heat.html",
        controller: "ChartHeatController",
        controllerAs: "chartHeat"
    };
}).controller("ChartHeatController", function ($rootScope, $scope, $http) {
    var bChart = this;
    bChart.range = RUNTIME_RANGE_DEFAULT;
    var domId = "chart_heat";
    var dataField = "heats";
    ...
    $scope.$watch(dataField, function () {
        //console.warn("heat $scope.$watch(" + dataField + ")");
        if (!$scope[dataField]) {
    ...
    bChart.initChart = function () {
        ...
        chart.graphs = [{
            id:"g1",
            valueField: "heat",
            balloonText: "热效率:[[value]] ",//"[[category]]<br><b><span style='font-size:14px;'>[[value]]</span></b>",
            bullet: "round",
            bulletSize: 2,
            lineColor: "#0D8ECF",
            lineThickness: 1,
    ...
    bChart.refreshChart = function (range) {
        ...
        switch (range) {
            ...
            case RUNTIME_RANGE_WEEK:
                ...
                Ladda.create(document.getElementById('chartHeat' + range)).start();
                $http.post('/boiler_runtime_list/', postData).then(function (res) {
                    ...
                    Ladda.create(document.getElementById('chartHeat' + range)).stop();
                });
                break;
